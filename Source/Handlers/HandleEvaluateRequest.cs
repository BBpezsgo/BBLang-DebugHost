        ...
        if (arguments.FrameId.HasValue)
        {
            ...
            foreach (var item in StackFrames)
            {
                ...
                foreach (var scope in item.Scopes)
                {
                    ...
                    {
                        if (variable.StackItem.Kind == StackElementKind.Internal) continue;

                        int address = variable.StackItem.AbsoluteAddress(Processor.Registers.BasePointer, Processor.StackStart);
                        ExpressionVariable v = new(variable.StackItem.Identifier, address, variable.StackItem.Type);
                        variables.Add(v);
                        Log.WriteLine(v);
                    }
                }
